...
var dal = function (el) {
  ...
  } else if (jslib(el).isString()) {
    self.DOM = document.getElementById(el) || (function () {
      var div = dal();
      div.DOM.id = el;
  ...
  // ## dal.move(opts)
  //
  // Makes the element positioned at `(opts.x, opts.y)`
  //
  // If the (optional) `opts.relative` is set to `true` the element is relative
  // positioned, else is absolute positioned

  self.move = function (opts) {
    self.DOM.style.position = opts.relative ? undefined : 'absolute';
