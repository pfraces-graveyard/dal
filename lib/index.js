...
var dal = function (el) {
  ...
  else if (typeof el === 'string') self.DOM = document.getElementById(el);
  else self.DOM = el; 
  ...
  self.children = function () {
    return Array.prototype.map.call(self.DOM.childNodes, function (el) {
      return el;
    });
  }
  ...
  self.attach = function () {
    if (!self.under(document.body)) document.body.appendChild(self.DOM);
    return self;
  }
  ...
  self.add = function (newEl, content) {
    var el = {};
    if (typeof newEl === 'string') el = document.createElement(newEl);
  ...
  self.del = function (target) {
    var el = self(target);
    if (el.under(self.DOM)) el.detach();
    return self;
  }
  ...
  self.clear = function () {
    while (!self.empty()) self.DOM.removeChild(self.DOM.firstChild);
    return self;
  }
